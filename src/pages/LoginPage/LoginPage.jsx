...
const LoginPage =({setIsDarkMode, isDarkMode})=>{
    ...
    const fetchLogininWithGoogle = useCallback(async () => {
        ...
        try {
            ...
            if (response.status) {
                setIsUserSettingExist(true); // setIsUserSettingExist 引用穩定
            } else {
                Swal.fire({ // Swal.fire 外部導入，引用穩定
                    title: "登入失敗",
    ...
    },[isValid, hasUserConfirmed, loginDisabled, navigate,fetchLogininWithGoogle])
